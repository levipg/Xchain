...
if [[ $cli ]]; then
  ...
  echo "Error: Please supply a xblockchain-cli in environment"
  exit 1
fi
...
cat > ${compose} << END
# Docker compose file for the demo xchain networks
version: '3'
...
cat >> ${compose} << END
    node${i}:
       ...
       networks:
         - private
       volumes:
         - node${i}_volume:/var/lib/xblockchain/data
         - ./genesis.json:/etc/xchain/genesis.json
         - ./nodes/${i}/:/var/lib/xblockchain/etc/
       environment:
         - RUST_BACKTRACE=1
       command:
         - /var/lib/xblockchain/bin/xchain_wrapper
         - ${nodeslist}
         - /var/lib/xblockchain/etc/config.yaml
         - /var/lib/xblockchain/bin/xchain
         - --config
         - /var/lib/xblockchain/etc/config.yaml.tmp
         - --genesis-config
         - /etc/xchain/genesis.json
         - --secret
         - /var/lib/xblockchain/etc/node_${i}.xprv
         - --storage
         - /var/lib/xblockchain/data
         - -vvv
END
